...
class TestBinanceExchange:
    ...
    def test_positions_unrealized_pnl_scaled_by_real_quantity(self, db_session, monkeypatch):
        ...
        from database import PaperOrder, Signal, Trade
        sig = Signal(id=3, symbol="BTCUSDT", side="BUY", approved=True)
        db_session.add(sig)
        db_session.flush()

        trade = Trade(id=30, signal_id=3, symbol="BTCUSDT", side="LONG", entry=50000.0, status="OPEN", pnl=100.0)
        db_session.add(trade)
        db_session.flush()

        order = PaperOrder(id=300, symbol="BTCUSDT", side="BUY", quantity=2.5, filled_quantity=2.5, status="FILLED", trade_id=30)
        db_session.add(order)
        db_session.flush()
