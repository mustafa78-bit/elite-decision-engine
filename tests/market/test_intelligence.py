...
class TestFearGreedService:
    ...
    def test_funding_rate_thresholds(self):
        # Mild / normal rate (e.g., 3.0) should not trigger any funding signals
        result_mild = self.service.compute(funding_rate=3.0)
        assert result_mild["value"] == 50
        assert "HIGH_FUNDING_GREED" not in result_mild["signals"]
        assert "NEGATIVE_FUNDING_FEAR" not in result_mild["signals"]

        # High positive rate (e.g., 25.0) should trigger HIGH_FUNDING_GREED with +10 score
        ...
        # Extreme positive rate (e.g., 55.0) should trigger HIGH_FUNDING_GREED with +20 score
        ...
        # High negative rate (e.g., -25.0) should trigger NEGATIVE_FUNDING_FEAR with -10 score
        ...
        # Extreme negative rate (e.g., -55.0) should trigger NEGATIVE_FUNDING_FEAR with -20 score
