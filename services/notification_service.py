...
class NotificationService:
    ...
    def list_notifications(
        ...
        try:
            ...
            return {
                ...
                "offset": offset,
                "limit": limit,
                "unread": session.query(Notification).filter(Notification.read.is_(False)).count(),
            }
        finally:
            session.close()
    ...
    def mark_all_read(self, event_type: str | None = None) -> int:
        session = self.session_factory()
        try:
            q = session.query(Notification).filter(Notification.read.is_(False))
            if event_type:
                q = q.filter(Notification.event_type == event_type)
            count = q.count()
            q.update({"read": True})
    ...
    def delete_all_read(self) -> int:
        session = self.session_factory()
        try:
            q = session.query(Notification).filter(Notification.read.is_(True))
            count = q.count()
            q.delete()
            session.commit()
            return count
