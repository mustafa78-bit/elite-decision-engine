...
// Lightweight price/intel header component to isolate fast WS-driven ticker re-renders.
function AssetHeader({ onExplainClick }: { onExplainClick: () => void }) {
  const { symbol } = useParams<{ symbol: string }>();
  const navigate = useNavigate();
  const { latestPrice, latestIntelligence } = useOutletContext<LayoutContext>();

  const currentSide = latestIntelligence
    ? latestIntelligence.decision === "STRONG_BUY" || latestIntelligence.decision === "BUY"
      ? "LONG"
      : latestIntelligence.decision === "STRONG_SELL" || latestIntelligence.decision === "SELL"
        ? "SHORT"
        : "NEUTRAL"
    : "NEUTRAL";
      ...
      <div className="flex items-center gap-3">
        ...
        {latestIntelligence && (
          <>
            ...
            <Badge variant={getDecisionBadge(latestIntelligence.decision).variant} className="text-[9px]">
              {getDecisionBadge(latestIntelligence.decision).label}
            </Badge>
          </>
        )}
      </div>
      <Button variant="primary" size="sm" onClick={onExplainClick}>
        Explain
      </Button>
    </div>
  );
...
// Lightweight market pulse card component to isolate rapid price tick re-renders.
function AssetMarketPulse() {
  const { latestPrice, latestIntelligence } = useOutletContext<LayoutContext>();
  const price = latestPrice?.price ?? 0;
  const change24h = latestPrice?.change_24h ?? 0;
...
// Heavy sections that change infrequently are extracted to sub-components that listen to context themselves.
function AssetScoreAndTimeline({ onExplainClick }: { onExplainClick: () => void }) {
  const { symbol } = useParams<{ symbol: string }>();
  const { latestIntelligence, notifications } = useOutletContext<LayoutContext>();
...
function AssetRightColumnWidgets() {
  const { symbol } = useParams<{ symbol: string }>();
  const { latestIntelligence, notifications } = useOutletContext<LayoutContext>();
...
function ExplainDrawerContainer({ open, onClose }: { open: boolean; onClose: () => void }) {
  const { symbol } = useParams<{ symbol: string }>();
...
export default function AssetDetail() {
  ...
  const handleExplainOpen = useCallback(() => setDrawerOpen(true), []);
  const handleExplainClose = useCallback(() => setDrawerOpen(false), []);

  return (
    <>
      <div className="space-y-6">
        <AssetHeader onExplainClick={handleExplainOpen} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="lg:col-span-2 space-y-4">
            ...
            <AssetScoreAndTimeline onExplainClick={handleExplainOpen} />
          </div>

          <div className="space-y-4">
            <AssetMarketPulse />
            <AssetRightColumnWidgets />
          </div>
        </div>
      </div>

      <ExplainDrawerContainer
        open={drawerOpen}
