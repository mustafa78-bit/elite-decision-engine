      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <OverviewCard label="BTC Status">
            <div className="flex items-center justify-between mb-2">
              <span className="text-lg font-bold tabular-nums text-[var(--text-primary)]">
                ${market.price != null ? market.price.toLocaleString(undefined, { minimumFractionDigits: 0 }) : "0"}
              </span>
              <RegimeBadge regime={market.regime} />
            </div>
            <div className="grid grid-cols-2 gap-1 text-xs">
              <div><span className="text-[var(--text-secondary)]">RSI</span> <span className="tabular-nums text-[var(--text-primary)] float-right">{market.rsi != null ? market.rsi.toFixed(0) : "0"}</span></div>
              <div><span className="text-[var(--text-secondary)]">Health</span> <span className="tabular-nums text-[var(--text-primary)] float-right">{market.btc_health_score != null ? (market.btc_health_score * 100).toFixed(0) : "0"}%</span></div>
            </div>
          </OverviewCard>
        )}
        ...
        {risk && (
          <OverviewCard label="Risk">
            <div className="text-2xl font-bold tabular-nums mb-2">
              <span className={(risk.risk_score || 0) >= 0.5 ? "text-[var(--accent-green)]" : "text-[var(--accent-red)]"}>
            ...
            <div className="grid grid-cols-2 gap-1 text-xs">
              ...
              <div><span className="text-[var(--text-secondary)]">Loss</span> <span className={`tabular-nums float-right ${(risk.daily_loss || 0) < 0 ? "text-[var(--accent-red)]" : "text-[var(--text-primary)]"}`}>${risk.daily_loss != null ? Math.abs(risk.daily_loss).toFixed(0) : "0"}</span></div>
            </div>
          </OverviewCard>
        )}
        ...
        {perf && (
          <OverviewCard label="Performance">
            <div className={`text-2xl font-bold tabular-nums mb-2 ${(totalPnl || 0) >= 0 ? "text-[var(--accent-green)]" : "text-[var(--accent-red)]"}`}>
              ${(totalPnl || 0).toFixed(0)}
            </div>
            <div className="grid grid-cols-2 gap-1 text-xs">
              ...
              <div><span className="text-[var(--text-secondary)]">Sharpe</span> <span className="tabular-nums text-[var(--text-primary)] float-right">{perf.sharpe_ratio != null ? perf.sharpe_ratio.toFixed(2) : "0"}</span></div>
            </div>
          </OverviewCard>
        )}
      </div>
