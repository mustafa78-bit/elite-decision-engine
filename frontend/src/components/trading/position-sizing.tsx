  ...
  return (
    <Card className="h-full">
      <CardHeader>
        ...
        <span className="text-[12px] font-mono text-[var(--text-muted)]">{symbol} @ ${price.toLocaleString()}</span>
      </CardHeader>
      <CardContent className="space-y-2">
        <div className="grid grid-cols-2 gap-2">
          <div className="space-y-1">
            <label className="text-[12px] font-mono text-[var(--text-muted)] uppercase tracking-wider">Account Size</label>
            <FormInput value={accountSize} onChange={(e) => setAccountSize(e.target.value)} className="h-7 text-[12px]" />
          </div>
          <div className="space-y-1">
            <label className="text-[12px] font-mono text-[var(--text-muted)] uppercase tracking-wider">Risk %</label>
            <FormInput value={riskPercent} onChange={(e) => setRiskPercent(e.target.value)} className="h-7 text-[12px]" />
          </div>
          <div className="space-y-1">
            <label className="text-[12px] font-mono text-[var(--text-muted)] uppercase tracking-wider">Stop Loss %</label>
            <FormInput value={stopLossPct} onChange={(e) => setStopLossPct(e.target.value)} className="h-7 text-[12px]" />
          </div>
          <div className="space-y-1">
            <label className="text-[12px] font-mono text-[var(--text-muted)] uppercase tracking-wider">Leverage</label>
            <FormInput value={leverage} onChange={(e) => setLeverage(e.target.value)} className="h-7 text-[12px]" />
          </div>
        </div>
