    ...
    <Card className="h-full">
      ...
      <CardContent className="space-y-2">
          ...
          <div key={m.name}>
            <div className="flex items-center justify-between mb-0.5">
              <div className="flex items-center gap-1.5">
                ...
                <Badge variant={m.prediction === "BULLISH" ? "success" : m.prediction === "BEARISH" ? "danger" : "warning"} className="text-[8px]">{m.prediction}</Badge>
              </div>
              <div className="flex items-center gap-1">
                <span className="text-[10px] font-mono tabular-nums text-[var(--accent-blue)]">{m.confidence}%</span>
                <span className="text-[8px] font-mono text-[var(--text-muted)]">w:{(m.weight * 100).toFixed(0)}%</span>
              </div>
            </div>
        ...
        <div className="pt-2 border-t border-[var(--border-subtle)] text-center">
          <span className="text-[9px] font-mono text-[var(--text-muted)]">{models.filter((m) => m.prediction === consensus).length}/{models.length} models agree</span>
        </div>
      </CardContent>
    </Card>
  );
