...
import { greetOLLO } from "../../api/ollo";
...
export function OLLOPanel() {
  ...
  // Active room mapping depending on the pathname
  const getActiveRoom = (path: string) => {
    if (path.startsWith("/scanner")) return "scanner";
    if (path.startsWith("/portfolio")) return "portfolio";
    if (path.startsWith("/risk")) return "risk_operations";
    if (path.startsWith("/whale")) return "whale";
    if (path.startsWith("/decisions") || path.startsWith("/signals")) return "council_chamber";
    if (path.startsWith("/timeline")) return "mission_archive";
    return "command_deck";
  };
  const activeRoom = getActiveRoom(location.pathname);

  // Fetch greeting whenever active room changes to greet the user contextually
  useEffect(() => {
    greetOLLO(activeRoom)
      .then((res) => {
        if (!res?.text) return;
        const greetMsg: RichOLLOMessage = {
          id: `greet-${activeRoom}-${Date.now()}`,
          sender: "ollo",
          timestamp: new Date().toISOString(),
          title: `OLLO OPERATOR: ${activeRoom.toUpperCase().replace("_", " ")}`,
          summary: res.text,
          text: res.text,
          reasoning: res.sections?.map(s => s.heading + (s.content ? ": " + s.content : "")) || ["Monitoring room signals", "Optimizing context metrics"]
        };
        // Insert greet as a new message or update initial
        setMessages(prev => {
          if (prev.some(m => m.id.startsWith(`greet-${activeRoom}`))) {
            return prev;
          }
      ...
      .catch((err) => {
        console.warn("Contextual greeting fetch failed, using default system prompts", err);
      });
  }, [activeRoom]);
  ...
  const getSuggestedQueries = (room: string) => {
    switch (room) {
      case "scanner":
        return [
          "Explain scanner signals",
          "Compare BTC vs SOL signals",
          "Summarize market conditions"
        ];
      case "whale":
        return [
          "Show whale activity",
          "Compare BTC vs SOL whale flow",
          "Summarize market conditions"
        ];
      case "portfolio":
        return [
          "Is my portfolio risk low?",
          "Show portfolio performance metrics",
          "Summarize market conditions"
        ];
      case "risk_operations":
        return [
          "Are there any risk limit alerts?",
          "Analyze current VaR risk",
          "Summarize market conditions"
        ];
      case "council_chamber":
        return [
          "What is the council consensus?",
          "Compare agent scores",
          "Explain BTC decision"
        ];
      case "mission_archive":
        return [
          "Show decision history",
          "Explain past trade decisions",
          "Summarize market conditions"
        ];
      default:
        return [
  ...
  const suggestedQueries = getSuggestedQueries(activeRoom);
