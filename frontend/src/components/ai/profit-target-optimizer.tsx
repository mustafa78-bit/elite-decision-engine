    ...
    <Card className="h-full">
      ...
      <CardContent className="space-y-2">
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="space-y-1">
            <label className="text-[12px] font-mono text-[var(--text-muted)]">Risk Amount ($)</label>
            <FormInput value={riskAmount} onChange={(e) => setRiskAmount(e.target.value)} className="h-7 text-[12px]" />
          </div>
        </div>
        <div className="space-y-1">
          <label className="text-[12px] font-mono text-[var(--text-muted)]">Target R:R</label>
          <FormInput value={riskReward} onChange={(e) => setRiskReward(e.target.value)} className="h-7 text-[12px]" />
        </div>
        <div className="space-y-1.5 pt-1">
          {targets.map((t) => (
            <div key={t.level} className="flex items-center justify-between px-2 py-1 rounded-lg bg-[var(--bg-base)] border border-[var(--border-subtle)] text-[12px] font-mono">
              <div className="flex items-center gap-2">
                <span className="text-[var(--text-secondary)]">{t.level}</span>
                <Badge variant={t.prob === "High" ? "success" : t.prob === "Medium" ? "warning" : "default"} className="text-[11px]">{t.prob}</Badge>
              </div>
              <div className="flex items-center gap-2">
