...
export default function RiskMonitor({ openTrades, maxOpenTrades }: Props) {
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded p-4">
      <h3 className="text-[10px] uppercase tracking-widest text-gray-500 mb-3">{t("riskMonitor.title")}</h3>
      <div className="text-xs">
        <div className="flex justify-between mb-1">
          <span className="text-gray-500">{t("riskMonitor.openPositions")}</span>
        ...
        <div className="text-[9px] text-gray-600 mt-1">{t("riskMonitor.capacityUsed", { pct: pct.toFixed(0) })}</div>
      </div>
    </div>
  );
}
